...
const ROUTE_WORDS = {
  ...
  greet: ['你好', '您好', 'hi', 'hello', '嗨', '在吗', '早上好', '早安', '晚上好', '晚安', '再见'],
...
function mountPet(ctx) {
  ...
  async function sendPetMsg() {
    ...
    if (r.layer === 1) {
      ...
      if (r.intent === 'greet') reply = pick(GREETINGS)
        ...
        reply = pick(ADVICE)
        showBubble('（悄悄探头）' + reply.slice(0, 26) + '…', 4000)
      }
      addPetChatMsg(reply)
    ...
    try {
      ...
      removeThink(thinkEl)
      addPetChatMsg('唔…信号不好，下次再聊嘛～（' + (e.message || '未知错误') + '）')
  ...
  // 后续任务替换的占位实现（本任务先落地，保证可运行）
  /** 2级占位（Task 5 替换）：无连接时快速失败，走道歉话术。 */
  async function fastChat() {
    throw new Error('2级暂未接入')
  }

  /** 3级占位（Task 6 替换）：先只播报，任务卡后续挂上。 */
  function createTaskCard(name) {
    showBubble('已收到任务「' + name.slice(0, 12) + '」！', 2500)
  }
