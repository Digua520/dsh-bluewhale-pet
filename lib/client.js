...
#dsh-pet-root {
  ...
  font-size: 12px; line-height: 1.4; color: #2a4450;
}
...
body[data-ds-dark-theme] #dsh-pet-root { color: #c8dce8; }
...
body[data-ds-dark-theme] #dsh-pet-root .pc-msg.pet { background: #273748; }
...
const ROUTE_WORDS = {
  // 任务意图由动作动词承载：'帮我/替我' 单独不触发（"帮我看看天气"是日常求助，不该进假管线）
  task: ['写', '修', '查', '分析', '运行', '跑一下', '生成', '整理', '监控', '审查', '部署', '测试'],
...
/** lookup 意图：短语级（任务/求助判定前）+ 单词级（其后，避免劫持"写个新闻爬虫"类任务）。 */
const LOOKUP_STRONG = ['天气预报', '天气怎么样', '天气如何', '多少度', '几度', '下雨吗', '会不会下雨', '搜一下']
const LOOKUP_TOPIC = ['天气', '气温', '温度']
const LOOKUP_QUERY = ['查', '看看', '搜', '预报', '怎么样', '如何']
const LOOKUP_WORD = ['新闻', '搜索', '汇率', '股价', '百科']
...
function route(text) {
  ...
  if (t.includes('帮帮我')) return { layer: 1, intent: 'help' }
  if (score(LOOKUP_STRONG) > 0 || (score(LOOKUP_TOPIC) > 0 && score(LOOKUP_QUERY) > 0)) return { layer: 2, intent: 'lookup' }
  if (score(ROUTE_WORDS.task) > 0) return { layer: 3, intent: 'task' }
  if (score(ROUTE_WORDS.help) > 0) return { layer: 1, intent: 'help' }
  if (score(LOOKUP_WORD) > 0) return { layer: 2, intent: 'lookup' }
...
function mountPet(ctx) {
  ...
  if (typeof window !== 'undefined' && window.__DSH_WHALE_TEST__) {
    ...
    window.__DSH_WHALE_TEST__.observerStart = () => observerStart()
    window.__DSH_WHALE_TEST__.tripleFire = tripleFire
    window.__DSH_WHALE_TEST__.tripleCount = () => tripleFired
  }
  ...
  function updateOverall() {
    ...
    if (tip) tip.textContent = (total - done) === 0 ? '任务都完成啦，蓝鲸待命中～' : '蓝鲸正在执行 ' + (total - done) + ' 个任务'
  }
  ...
  // ---- Ambient Observer：主屏求助信号侦测（探头三连） ----
  // detectSignal / SIGNAL_PATTERNS 为纯函数，定义在工厂作用域（见 apply 之后），
  // 供 loadPet 阶段 seam 直接挂载（测试不 apply 即可断言）。
  const FIX_PROMPTS = {
    stuck: '请帮我排查任务卡住问题：① 检查是否有工具在等待确认 ② 检查模型输出是否超时 ③ 检查是否有命令挂起。',
    error: '请帮我分析这个报错：提取错误栈关键行，检查路径、版本、权限三类原因。',
    help: '请帮我拆解当前任务：① 列出步骤 ② 检查项目结构 ③ 跑相关测试验证。',
  }

  let tripleFired = 0
  let lastFireTick = -100 // 上次触发所在的轮询节拍（防抖时间轴见 scanMain 注释）
  let lastSignalTail = null // 上次触发时的信号文案尾：同文案不重复报
  let scanTimer = null
  let observer = null
  let pollTimer = null
  let pollTick = 0 // 轮询节拍计数（一拍 = 500ms）

  const DEBOUNCE_TICKS = 6 // 3s 防抖窗口 ÷ 500ms 扫描拍

  function tripleFire(kind, snippet) {
    tripleFired += 1
    // 旧 8s 自动隐藏计时器先清掉：多次触发时防止旧计时器提前隐藏新气泡
    clearTimeout(bubble._hideFix)
    // 先唤醒再建气泡：wake 的「睡醒了」气泡会被下方的三连气泡覆盖（气泡最后写入必胜）
    wake()
    // ① 气泡探头 + 【排查】按钮
    const bubbleText = '发现异常' + (kind === 'error' ? '报错' : kind === 'stuck' ? '卡住' : '求助') + '信号，需要小蓝帮你分析吗？🫧'
    bubble.textContent = ''
    bubble.innerHTML = bubbleText + ' <button class="pc-fix-btn pxfont" style="margin-left:4px">排查</button>'
    clearTimeout(bubble._t)
    bubble.style.display = 'block'
    if (hp) hp.style.display = 'none'
    if (stateEl) stateEl.style.display = 'none'
    const btn = bubble.querySelector('.pc-fix-btn')
    btn.addEventListener('click', () => {
      // ③ 主屏注入（红线：仅用户点击后）
      const injectPrompt = (el, prompt) => {
    ...
    // 三连气泡独立 8s 自动隐藏（不被挂载提示 2.5s 计时器提前藏掉）
    bubble._hideFix = setTimeout(() => {
    ...
    // ② 面板记录
    addPetChatMsg('【主屏信号】' + (snippet || kind) + ' → 已记录，点气泡【排查】获取排查提示')
  }

  function scanMain() {
    let main = null
    for (const sel of SCAN_TARGETS) {
      main = document.querySelector(sel)
      if (main) break
    }
    if (!main) return
    // 有界尾部读取：从最后子节点倒着收集到 limit 长度，O(尾部)
    const tailText = (node, limit) => {
      const parts = []
    ...
    const text = tailText(main, 600)
    const kind = detectSignal(text.slice(-600))
    if (!kind) { lastSignalTail = null; return }
    const tail = text.slice(-120)
    if (tail === lastSignalTail) return // 同信号文案不重复报（不因文案常驻而周期性刷屏）
    // 防抖 3s：以轮询节拍计（一拍 = 500ms，DEBOUNCE_TICKS 拍 = 3s）。不用 Date.now()——
    // jsdom 窗口域 Date 不被假时钟接管，假时钟测试下永不过期。
    if (pollTick - lastFireTick < DEBOUNCE_TICKS) return
    lastFireTick = pollTick
    lastSignalTail = tail
    tripleFire(kind, tail)
  }

  // 主聊天区容器选择器，真机验证后可按宿主 DOM 调整
  const SCAN_TARGETS = ['.chat-content', 'main']

  function observerStart() {
    if (observer) return
    observer = new MutationObserver(() => {
      // throttle 500ms：定时扫描合并高频变更
      if (scanTimer) return
    ...
    // 兜底轮询：jsdom/假时钟下 MutationObserver 的微任务送达与扫描节拍不同步
    // （微任务不被同步 tick 冲刷），以 500ms 周期扫描兜底并驱动防抖节拍；
    // 与 mutation 触发的 scanTimer 由同一节流合并，不重复扫描。
  ...
  async function sendPetMsg() {
    ...
    setThink(thinkEl, '…')
    try {
      const text = await fastChat(v, r.intent === 'lookup')
      removeThink(thinkEl)
      addPetChatMsg(text)
      logTurn(v, r, Date.now() - started, true)
    } catch (e) {
  ...
  $('dsh-pet-input').addEventListener('keydown', (e) => { if (e.key === 'Enter' && !e.isComposing) sendPetMsg() })

  /** 2级：经 /bluewhale-pet 通道请求 host LLM（unary RPC；fast-chat 10s / lookup 15s 超时，搜索+合成更慢）。 */
  async function fastChat(text, lookup) {
    const rpc = ctx && ctx.connection && ctx.connection.rpc
    if (!rpc) throw new Error('连接服务不可用')
    const ac = new AbortController()
    const timer = setTimeout(() => ac.abort(), lookup ? 15000 : 10000)
    try {
      const res = await rpc.call('/bluewhale-pet', lookup ? 'lookup' : 'fast-chat', { text }, ac.signal)
  ...
  /** 3级：新建任务卡（RUNNING 态，▶ 0%），交给假管线推进；v1 仅 doing/done 两态，PENDING/THINKING 随真 subagent 接入（v1.x）。 */
  ...
  const LOG_KEY = 'dsh-bluewhale:chatlog'
  const LOG_MAX = 200

  /** 数据飞轮：路由日志环形缓冲（仅记录，不加 UI）。 */
  function logTurn(text, r, latency, ok) {
    let logs = []
    try { logs = JSON.parse(localStorage.getItem(LOG_KEY) || '[]') } catch {}
    if (!Array.isArray(logs)) logs = []
    logs.push({ t: Date.now(), text: text.slice(0, 60), layer: r.layer, intent: r.intent, latency, ok })
    if (logs.length > LOG_MAX) logs = logs.slice(logs.length - LOG_MAX)
    // 配额满等写入失败时静默丢弃本轮日志（有意行为：遥测不阻断聊天）
    try { localStorage.setItem(LOG_KEY, JSON.stringify(logs)) } catch {}
  }
  if (typeof window !== 'undefined' && window.__DSH_WHALE_TEST__) {
    window.__DSH_WHALE_TEST__.logTurn = logTurn
  }
  ...
  function setPetTheme(night) {
    ...
    if (ctx && ctx.theme) {
      try { ctx.theme.setTheme(night ? 'sunny-pixel-dark' : 'sunny-pixel') } catch (e) {
        console.warn('[dsh-bluewhale-pet] setTheme failed:', e)
      }
    }
  ...
  } catch (e) { console.warn('[dsh-bluewhale-pet] restore state failed:', e) }
  ...
  // 串行队列语义：同一时刻只推进最早挂载的 doing 任务，其余排队等待
  ...
  function spawnConfetti() {
    ...
    const room = 60 - existing.length
    for (let i = 0; i < Math.min(24, room); i++) {
...
function apply(ctx) {
  ...
  // 主题注册：宿主支持 register 时双主题进入 Appearance；不阻塞挂载
  ctx.effect(() => {
    try {
      ctx.theme.register({ id: 'sunny-pixel', colorScheme: 'light', tokens: LIGHT_TOKENS })
      ctx.theme.register({ id: 'sunny-pixel-dark', colorScheme: 'dark', tokens: DARK_TOKENS })
    } catch (e) {
      console.warn('[dsh-bluewhale-pet] theme.register failed:', e)
    }
  }, 'dsh-bluewhale-pet: theme register')
  ...
  // 测试契约：mockCtx 的 effect 同步执行，返回真实 disposer；真实 cordis 为延迟执行、
  // 由 effect 自身的 disposer 负责清理，此返回值在宿主路径不消费。
  return cleanup
}

    // Ambient Observer 纯函数（mountPet 内 observerStart/tripleFire 复用；独立于 mount，
    // 供 seam 在 loadPet 阶段即挂载——测试不 apply 也可断言 detectSignal）
    const SIGNAL_PATTERNS = {
      stuck: [/卡住/, /不动/, /没反应/],
      error: [/failed/i, /error/i, /报错/, /异常/, /崩溃/],
      help: [/怎么办/, /帮帮我/, /怎么写/],
    }

    function detectSignal(text) {
      if (typeof text !== 'string' || !text) return null
      for (const [kind, patterns] of Object.entries(SIGNAL_PATTERNS)) {
    ...
    if (typeof window !== 'undefined' && window.__DSH_WHALE_TEST__) {
      ...
      window.__DSH_WHALE_TEST__.detectSignal = detectSignal
    }
