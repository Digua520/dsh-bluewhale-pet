...
function mountPet(ctx) {
  ...
  function tripleFire(kind, snippet) {
    ...
    btn.addEventListener('click', () => {
      ...
      if (mainInput && mainInput.tagName === 'TEXTAREA') {
        mainInput.value = (mainInput.value ? mainInput.value + '\n' : '') + prompt
        mainInput.dispatchEvent(new Event('input', { bubbles: true }))
      } else if (mainInput) {
        mainInput.textContent = prompt
      } else {
        // 兜底：复制到剪贴板，绝不直接写主聊天流
        try { navigator.clipboard.writeText(prompt) } catch {}
      }
      clearTimeout(hideFix)
      bubble.style.display = 'none'
      if (hp) hp.style.display = ''
      if (stateEl) stateEl.style.display = stateEl.textContent === '开心' ? 'none' : ''
    })
    // 三连气泡独立 8s 自动隐藏（不被挂载提示 2.5s 计时器提前藏掉）
    const hideFix = setTimeout(() => {
  ...
  function scanMain() {
    ...
    // 防抖 3s：以轮询节拍计（一拍 = 500ms，6 拍 = 3s）。不用 Date.now()——
    // jsdom 窗口域 Date 不被假时钟接管，假时钟测试下永不过期。
    if (pollTick - lastFireTick < 6) return
